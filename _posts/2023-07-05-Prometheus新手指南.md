...
PromQL主要有四种数据类型：瞬时向量（Instant Vector）、区间向量（Range Vector）、标量（Scalar）和字符串（String）。

- **瞬时向量**：瞬时向量是一组时间序列的当前值，每个时间序列包括一个指标名称、一组标签和一个浮点值。例如，`http_requests_total`返回的就是一个瞬时向量。

- **区间向量**：区间向量是一组时间序列的一段时间范围内的值，每个时间序列包括一个指标名称、一组标签和一组数据点。每个数据点包括一个时间戳和一个浮点值。例如，`http_requests_total[5m]`返回的就是一个区间向量，包含了过去5分钟内的所有数据。
